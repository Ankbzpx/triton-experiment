...
def chamfer_distance_sp(ref_pts, query_pts):
    ...
    return (
        ref_closest_dist_sp,
        ref_closest_index_sp,
        query_closest_dist_sp,
        query_closest_index_sp,
    )
...
def get_cuda_autotune_config():
    return [
        triton.Config(
            {"BLOCK_SIZE_N": 16, "BLOCK_SIZE_M": 512, "GROUP_SIZE": 1},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
        ...
        triton.Config(
            {"BLOCK_SIZE_N": 16, "BLOCK_SIZE_M": 512, "GROUP_SIZE": 16},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
        ...
        triton.Config(
            {"BLOCK_SIZE_N": 32, "BLOCK_SIZE_M": 512, "GROUP_SIZE": 16},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
        ...
        triton.Config(
            {"BLOCK_SIZE_N": 32, "BLOCK_SIZE_M": 512, "GROUP_SIZE": 32},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
        ...
        triton.Config(
            {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_M": 512, "GROUP_SIZE": 16},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
        ...
        triton.Config(
            {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_M": 512, "GROUP_SIZE": 32},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
        ...
        triton.Config(
            {"BLOCK_SIZE_N": 64, "BLOCK_SIZE_M": 512, "GROUP_SIZE": 64},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
...
def nm_dist_kernel(
    ...
    xyz1_x = tl.load(
        xyz1_ptr + batch_base_n * xyz1_stride_n, mask=batch_n_mask, other=100
    )
    xyz1_y = tl.load(
        xyz1_ptr + batch_base_n * xyz1_stride_n + xyz1_stride_d,
        mask=batch_n_mask,
        other=100,
    )
    xyz1_z = tl.load(
        xyz1_ptr + batch_base_n * xyz1_stride_n + 2 * xyz1_stride_d,
        mask=batch_n_mask,
        other=100,
    )

    cur_best_d = tl.full((BLOCK_SIZE_N,), float("inf"), tl.float32)
    cur_best_idx = tl.zeros((BLOCK_SIZE_N,), tl.int32)
    for base_m in tl.range(0, M, BLOCK_SIZE_M):
        batch_base_m = base_m + tl.arange(0, BLOCK_SIZE_M)
        batch_m_mask = batch_base_m < M
        xyz2_x = tl.load(
            xyz2_ptr + batch_base_m * xyz2_stride_m, mask=batch_m_mask, other=-100
        )
        xyz2_y = tl.load(
            xyz2_ptr + batch_base_m * xyz2_stride_m + xyz2_stride_d,
            mask=batch_m_mask,
            other=-100,
        )
        xyz2_z = tl.load(
            xyz2_ptr + batch_base_m * xyz2_stride_m + 2 * xyz2_stride_d,
            mask=batch_m_mask,
            other=-100,
        )
    ...
    tl.store(dists_ptr + batch_base_n * dist_stride_n, cur_best_d, mask=batch_n_mask)
    tl.store(
        indices_ptr + batch_base_n * indices_stride_n, cur_best_idx, mask=batch_n_mask
    )
...
def _(xyz1: torch.Tensor, xyz2: torch.Tensor):
    ...
    return (
        xyz1.new_empty((N,)),
        xyz1.new_empty((N,), dtype=torch.int32),
        xyz2.new_empty((M,)),
        xyz2.new_empty((M,), dtype=torch.int32),
    )
...
def chamfer_distance_backward(ctx, grad_out):
    ...
    grad_xyz1 = torch.scatter_add(
        d_dist1, 0, idx2[:, None].expand(-1, 3).long(), -d_dist2
    )
    grad_xyz2 = torch.scatter_add(
        d_dist2, 0, idx1[:, None].expand(-1, 3).long(), -d_dist1
    )
...
def chamfer_distance_kd(xyz1: torch.Tensor, xyz2: torch.Tensor) -> List[torch.Tensor]:
